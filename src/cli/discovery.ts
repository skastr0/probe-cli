...
export const schemaEntries: ReadonlyArray<SchemaDiscoveryEntry> = [
  ...
  },
  {
    ...
    description: "Perform one UI action. CLI also accepts actions[] as a fly path (rewrites to session.run sequence; not an RPC field).",
    input_controls: [...jsonInputControls],
    output_controls: [...outputControls],
    payload_shape: {
      sessionId: "string optional when --session-id is provided",
      action: "SessionAction (RPC session.action params)",
      "actions (CLI-only)": "SessionAction[] — CLI rewrites to session.run fast sequence with sparse evidence; not part of session.action RPC",
    },
    result_shape: { result: "SessionActionResult (RPC) | SessionFlowResult (CLI actions[] path only)" },
  },
  {
    schema_id: "probe.command.session.recording.export.input/v1",
    command: "session recording export",
    rpc_method: "session.recording.export",
    description: "Export recorded actions as script-v1 (RPC). CLI --format flow-v2 converts the written script locally to session-flow/v2 (not an RPC param).",
    input_controls: ["--session-id <id>", "--label <label>", "--format script|flow-v2 (CLI-only)"],
    output_controls: [...outputControls],
    payload_shape: {
      sessionId: "string (RPC)",
      label: "string optional (RPC)",
      "format (CLI-only)": "script (default) | flow-v2 — flow-v2 is post-process after script export; fails closed on bare preferredRef-only steps",
    },
    result_shape: { result: "SessionRecordingExportResult (RPC script) | CLI flow-v2 wrapper with flowPath + scriptArtifact" },
  },
  {
...
export const exampleEntries: ReadonlyArray<ExampleDiscoveryEntry> = [
  ...
  },
  {
    ...
    command: "session action",
    description: "Tap a known accessibility identifier (runner-direct, no host snapshot tax).",
    invocation: "probe session action --input-json '{\"sessionId\":\"<session-id>\",\"action\":{\"kind\":\"tap\",\"target\":{\"kind\":\"semantic\",\"identifier\":\"fixture.form.applyButton\",\"label\":null,\"value\":null,\"placeholder\":null,\"type\":null,\"section\":null,\"interactive\":true},\"evidencePolicy\":{\"success\":\"none\",\"failure\":\"none\"}}}' --output-json",
    payload: {
  ...
  {
    name: "session-action-multitap-json",
    command: "session action",
    description: "Five discrete taps in one RPC (passcode/unlock style). Prefer multiTap over five separate taps.",
    invocation: "probe session action --input-json '{\"sessionId\":\"<session-id>\",\"action\":{\"kind\":\"multiTap\",\"target\":{\"kind\":\"semantic\",\"identifier\":\"fixture.gesture.multiTapTarget\",\"label\":null,\"value\":null,\"placeholder\":null,\"type\":null,\"section\":null,\"interactive\":true},\"tapCount\":5,\"interTapDelayMs\":40,\"evidencePolicy\":{\"success\":\"none\",\"failure\":\"none\"}}}' --output-json",
    payload: {
      sessionId: "<session-id>",
      action: {
        kind: "multiTap",
  ...
  {
    name: "session-action-point-multitap-json",
    command: "session action",
    description: "Tap a screen pixel five times quickly (point multiTap — no AX resolution).",
    invocation: "probe session action --input-json '{\"sessionId\":\"<session-id>\",\"action\":{\"kind\":\"multiTap\",\"target\":{\"kind\":\"point\",\"x\":200,\"y\":400},\"tapCount\":5,\"interTapDelayMs\":40,\"evidencePolicy\":{\"success\":\"none\",\"failure\":\"none\"}}}' --output-json",
    payload: {
      sessionId: "<session-id>",
      action: {
        kind: "multiTap",
        target: { kind: "point", x: 200, y: 400 },
        tapCount: 5,
        interTapDelayMs: 40,
        evidencePolicy: { success: "none", failure: "none" },
      },
  ...
  {
    name: "session-action-batch-json",
    command: "session action",
    description: "Batch multiple mutations in one call (host expands to a fast sequence / uiActionBatch).",
    invocation: "probe session action --input-json '{\"sessionId\":\"<session-id>\",\"actions\":[{\"kind\":\"type\",\"target\":{\"kind\":\"semantic\",\"identifier\":\"fixture.form.input\",\"label\":null,\"value\":null,\"placeholder\":null,\"type\":\"textField\",\"section\":null,\"interactive\":true},\"text\":\"hello\",\"replace\":true},{\"kind\":\"tap\",\"target\":{\"kind\":\"semantic\",\"identifier\":\"fixture.form.applyButton\",\"label\":null,\"value\":null,\"placeholder\":null,\"type\":null,\"section\":null,\"interactive\":true}}]}' --output-json",
    payload: {
      sessionId: "<session-id>",
      actions: [
        {
  ...
  {
    name: "agent-navigation-playbook",
    command: "session action",
    description: "Read docs/examples/flows/agent-navigation.md for the full fly-through playbook (multiTap, sequence, evidence-none).",
    invocation: "cat docs/examples/flows/agent-navigation.md",
    payload: null,
  },
  {
