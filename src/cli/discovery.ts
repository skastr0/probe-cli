...
export const schemaEntries: ReadonlyArray<SchemaDiscoveryEntry> = [
  ...
  },
  {
    ...
    description: "Perform one UI action, or a batched actions[] fly path (fast sequence, sparse evidence by default).",
    input_controls: [...jsonInputControls],
    output_controls: [...outputControls],
    payload_shape: {
      sessionId: "string optional when --session-id is provided",
      action: "SessionAction optional (mutually exclusive with actions)",
      actions: "SessionAction[] optional — agent fly path: one fast sequence, evidencePolicy success=none unless set on the sequence layer by CLI",
    },
    result_shape: { result: "SessionActionResult | SessionFlowResult (when actions[] is used)" },
  },
  {
    ...
    description: "Export recorded session actions as script-v1, or convert to session-flow/v2 for CI re-run.",
    input_controls: ["--session-id <id>", "--label <label>", "--format script|flow-v2"],
    output_controls: [...outputControls],
    payload_shape: { sessionId: "string", label: "string optional", format: "script (default) | flow-v2" },
    result_shape: { result: "SessionRecordingExportResult | { flowPath, flow, scriptArtifact, stepCount }" },
  },
  {
