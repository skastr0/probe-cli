...
describe("extended metal analysis", () => {
  test("combines gpu, driver, and encoder exports into one summary", () => {
    ...
    expect(result.summary.headline).toContain("60.6 fps GPU frame-span avg")
    expect(result.summary.headline).toContain("1 of 2 frames exceeded")
    expect(result.summary.metrics.find((metric) => metric.label === "GPU frame-span FPS")?.value).toBe("60.6 fps")
    expect(result.summary.metrics.find((metric) => metric.label === "Frames over 60Hz reference")?.value).toBe("1/2")
