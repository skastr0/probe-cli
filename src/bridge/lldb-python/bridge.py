...
def _process_snapshot(process: lldb.SBProcess, frame_limit: int = 8) -> dict[str, Any]:
    ...
    return {
        ...
        "selectedThread": {
            ...
            "stopDescription": selected_thread.GetStopDescription(256),
        }
...
def _breakpoint_location_to_dict(location: lldb.SBBreakpointLocation) -> dict[str, Any]:
    ...
    return {
        ...
        "loadAddress": address.GetLoadAddress(lldb.SBTarget()),
        "lineEntry": line_info,
    }


def _breakpoint_to_dict(breakpoint: lldb.SBBreakpoint) -> dict[str, Any]:
    locations = [
        _breakpoint_location_to_dict(breakpoint.GetLocationAtIndex(index))
    ...
    return {
        ...
        "isOneShot": breakpoint.IsOneShot(),
        "numLocations": breakpoint.GetNumLocations(),
        "numResolvedLocations": breakpoint.GetNumResolvedLocations(),
...
class BridgeSession:
    ...
    def _require_target(self) -> lldb.SBTarget:
    ...
    def set_breakpoint(self, request: dict[str, Any]) -> dict[str, Any]:
        target = self._require_target()

        location_kind = str(request.get("locationKind") or "")

        if location_kind == "function":
            function_name = str(request.get("functionName") or "")
            if function_name == "":
                raise RuntimeError("Breakpoint functionName is required.")
            breakpoint = target.BreakpointCreateByName(function_name)
        elif location_kind == "file-line":
            file_path = str(request.get("file") or "")
            line = int(request.get("line") or 0)
            if file_path == "" or line <= 0:
                raise RuntimeError("Breakpoint file and positive line are required.")
            breakpoint = target.BreakpointCreateByLocation(file_path, line)
        else:
            raise RuntimeError(f"Unknown breakpoint location kind: {location_kind}")

        if not breakpoint.IsValid() or breakpoint.GetNumLocations() == 0:
            raise RuntimeError("Breakpoint did not resolve any locations.")

        return {
            "process": _process_snapshot(self._require_process()),
            "breakpoint": _breakpoint_to_dict(breakpoint),
        }

    def clear_breakpoint(self, breakpoint_id: int) -> dict[str, Any]:
        target = self._require_target()
        deleted = target.BreakpointDelete(breakpoint_id)
        if not deleted:
            raise RuntimeError(f"Breakpoint {breakpoint_id} could not be deleted.")
    ...
    try:
        for raw_line in sys.stdin:
            ...
            try:
                ...
                elif command == "breakpoint-set":
                    response = _success_response(
                        request_id,
                        command,
                        session.set_breakpoint(request),
                    )
                elif command == "breakpoint-clear":
                    response = _success_response(
                        request_id,
                        command,
                        session.clear_breakpoint(int(request["breakpointId"])),
                    )
